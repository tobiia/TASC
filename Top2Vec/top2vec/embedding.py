...
def average_embeddings(
    ...
    model = AutoModel.from_pretrained(
        embedding_model, output_hidden_states=embedding_layer is not None
    )
...
def sliding_window_average(
    document_token_embeddings, document_tokens, window_size, stride
):
...
def smooth_document_token_embeddings(document_token_embeddings, window_size=2):
    ...
    for doc in tqdm(
        document_token_embeddings, desc="Smoothing document token embeddings"
    ):
