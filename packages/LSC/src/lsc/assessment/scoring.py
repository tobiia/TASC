...
import numpy as np
import pandas as pd
from lsc.utils import l2_normalize, load_gold_set, apd, prt, spearmans
from lsc.representation.embed_cache import run_cache as run_embed_cache
from lsc.extraction.word_cache import run_cache as run_word_cache
from lsc.config import RESULTS_DIR
...
def main():
    ...
    output_path = RESULTS_DIR / f"eval_{args.label if args.label else ''}.csv"
    ...
    for model_name, layer in product(args.models, LAYERS):
        ...
        try:
            (x_embeds, _, _), (y_embeds, _, _) = run_embed_cache(
