...
top2vec_docids = Top2Vec(
    documents=newsgroups_documents, document_ids=doc_ids, speed="fast-learn", workers=8
)

# train top2vec model without saving documents
top2vec_no_docs = Top2Vec(
    documents=newsgroups_documents, keep_documents=False, speed="fast-learn", workers=8
)

# train top2vec model with corpus_file
top2vec_corpus_file = Top2Vec(
    documents=newsgroups_documents, use_corpus_file=True, speed="fast-learn", workers=8
)

# test USE
top2vec_use = Top2Vec(
    documents=newsgroups_documents, embedding_model="universal-sentence-encoder"
)
...
top2vec_transformer_multilang = Top2Vec(
    documents=newsgroups_documents, embedding_model="distiluse-base-multilingual-cased"
)

# test Sentence Transformer with model embedding
top2vec_transformer_model_embedding = Top2Vec(
    documents=newsgroups_documents,
    embedding_model="distiluse-base-multilingual-cased",
    use_embedding_model_tokenizer=True,
)

top2vec_transformer_use_large = Top2Vec(
    documents=newsgroups_documents,
...
top2vec_transformer_use_multilang_large = Top2Vec(
    documents=newsgroups_documents,
...
top2vec_transformer_sbert_l6 = Top2Vec(
    documents=newsgroups_documents,
...
top2vec_transformer_sbert_l12 = Top2Vec(
    documents=newsgroups_documents,
...
top2vec_model_callable = Top2Vec(
    documents=newsgroups_documents,
...
top2vec_ngrams = Top2Vec(
    documents=newsgroups_documents, speed="fast-learn", ngram_vocab=True, workers=8
)

top2vec_use_ngrams = Top2Vec(
    documents=newsgroups_documents,
    embedding_model="universal-sentence-encoder",
    ngram_vocab=True,
)
...
models = [
    ...
    top2vec_transformer_multilang,
    top2vec_use_model_embedding,
    top2vec_transformer_model_embedding,
...
def test_add_documents_original(top2vec_model):
    ...
    assert (
        topic_count_sum + len(docs_to_add)
        == topic_count_sum_new
        == num_docs + len(docs_to_add)
        == num_docs_new
        == len(top2vec_model.doc_top)
    )
...
def test_compute_topics(top2vec_model):
    ...
    assert all(
        topic_words_scores[i] >= topic_words_scores[i + 1]
        for i in range(len(topic_words_scores) - 1)
    )
...
def test_add_documents_post_reduce(top2vec_model):
    ...
    assert (
        topic_count_sum + len(docs_to_add)
        == topic_count_sum_new
        == topic_count_reduced_sum + len(docs_to_add)
        == topic_count_reduced_sum_new
        == num_docs + len(docs_to_add)
        == num_docs_new
        == len(top2vec_model.doc_top)
...
def test_delete_documents(top2vec_model):
    ...
    assert (
        topic_count_sum - len(doc_ids_to_delete)
        == topic_count_sum_new
        == topic_count_reduced_sum - len(doc_ids_to_delete)
        == topic_count_reduced_sum_new
        == num_docs - len(doc_ids_to_delete)
        == num_docs_new
        == len(top2vec_model.doc_top)
        == len(top2vec_model.doc_top_reduced)
    )
...
def test_get_topic_size(top2vec_model, reduced):
    ...
    assert all(
        topic_sizes[i] >= topic_sizes[i + 1] for i in range(len(topic_sizes) - 1)
    )
...
def test_search_documents_by_topic(top2vec_model, reduced):
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_topic(topic, num_docs, reduced=reduced)
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_topic(
            topic, num_docs, reduced=reduced
        )
    ...
    assert all(
        document_scores[i] >= document_scores[i + 1]
        for i in range(len(document_scores) - 1)
    )
    ...
    if reduced:
        doc_topics = set(
            np.argmax(
                np.inner(
                    top2vec_model.document_vectors[document_indexes],
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_keywords(
                keywords=[keyword], num_docs=num_docs
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_keywords(
            keywords=[keyword], num_docs=num_docs
        )
...
def test_similar_words(top2vec_model):
    ...
    words, word_scores = top2vec_model.similar_words(
        keywords=[keyword], num_words=num_words
    )
    ...
    assert all(
        word_scores[i] >= word_scores[i + 1] for i in range(len(word_scores) - 1)
    )
...
def test_search_topics(top2vec_model, reduced):
    ...
    topic_words, word_scores, topic_scores, topic_nums = top2vec_model.search_topics(
        keywords=[keyword], num_topics=num_topics, reduced=reduced
    )
    ...
    assert all(
        topic_scores[i] >= topic_scores[i + 1] for i in range(len(topic_scores) - 1)
    )
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_documents(
                doc_ids=[doc_id], num_docs=num_docs
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_documents(
            doc_ids=[doc_id], num_docs=num_docs
        )
...
def test_get_documents_topics(top2vec_model):
    ...
    if top2vec_model.hierarchy is not None:
        doc_topics, doc_dist, topic_words, topic_word_scores = (
            top2vec_model.get_documents_topics(doc_ids=doc_ids_get, reduced=True)
        )
    else:
        doc_topics, doc_dist, topic_words, topic_word_scores = (
            top2vec_model.get_documents_topics(doc_ids=doc_ids_get)
        )
...
def test_get_documents_topics_multiple(top2vec_model):
    ...
    if top2vec_model.hierarchy is not None:
        doc_topics, doc_dist, topic_words, topic_word_scores = (
            top2vec_model.get_documents_topics(
                doc_ids=doc_ids_get, reduced=True, num_topics=num_topics
            )
        )
        ...
        doc_topics, doc_dist, topic_words, topic_word_scores = (
            top2vec_model.get_documents_topics(
                doc_ids=doc_ids_get, num_topics=num_topics
            )
        )
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_vector(
                vector=document_vectors[0], num_docs=num_docs
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_vector(
            vector=document_vectors[0], num_docs=num_docs
        )
    if top2vec_model.documents is not None:
        assert len(documents) == len(document_scores) == len(document_ids) == num_docs
...
def test_index_documents(top2vec_model):
    ...
    assert (
        top2vec_model.document_vectors.shape[1]
        <= top2vec_model.document_index.get_max_elements()
    )
...
def test_search_documents_by_vector_index(top2vec_model):
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_vector(
                vector=document_vectors[0], num_docs=num_docs, use_index=True
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_vector(
...
def test_search_documents_by_keywords_index(top2vec_model):
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_keywords(
                keywords=[keyword], num_docs=num_docs, use_index=True
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_keywords(
...
def test_search_document_by_documents_index(top2vec_model):
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = (
            top2vec_model.search_documents_by_documents(
                doc_ids=[doc_id], num_docs=num_docs, use_index=True
            )
        )
    else:
        document_scores, document_ids = top2vec_model.search_documents_by_documents(
...
def test_search_words_by_vector(top2vec_model):
    ...
    words, word_scores = top2vec_model.search_words_by_vector(
        vector=word_vectors[0], num_words=num_words
    )
...
def test_index_words(top2vec_model):
    ...
    assert (
        top2vec_model.word_vectors.shape[1]
        <= top2vec_model.word_index.get_max_elements()
    )
...
def test_similar_words_index(top2vec_model):
    ...
    words, word_scores = top2vec_model.similar_words(
        keywords=[keyword], num_words=num_words, use_index=True
    )
...
def test_query_documents(top2vec_model):
    ...
    if top2vec_model.documents is not None:
        documents, document_scores, document_ids = top2vec_model.query_documents(
            query="what is the meaning of life?", num_docs=num_docs
        )
    else:
        document_scores, document_ids = top2vec_model.query_documents(
            query="what is the meaning of life?", num_docs=num_docs
        )
...
def test_query_topics(top2vec_model):
    num_topics = top2vec_model.get_num_topics()
    topic_words, word_scores, topic_scores, topic_nums = top2vec_model.query_topics(
        query="what is the " "meaning of life?", num_topics=num_topics
    )
