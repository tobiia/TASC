...
DOMAIN = "corp"  # corp, equi, htfl, wind
GRAM_TYPE = "ngram"  # uni, ngram, all
USE_CACHE = True
CORPUS_PATH = (
...
def calculate_metrics(true_terms: set, extracted_terms: set, print_results=False):
...
def load_gold_set(path, wanted_set):
    ...
    # NOTE hyphenated terms are considered mwes
    true_terms_mwe = set(
...
# ---------------------------------------------------------------------------
# ANCHOR testing the candidate extractors
...
def candidate_test():
    ...
    """ uni_gold = load_gold_set(str(GOLD_SET_PATH), "uni")
    mwe_gold = load_gold_set(str(GOLD_SET_PATH), "ngram") """
    gold = load_gold_set(str(GOLD_SET_PATH), "all")

    """ uni_candidates = set(extractor.unigram_candidates()) # unigrams only
    calculate_metrics(uni_gold, uni_candidates, True) """
    """ ngram_candidates = set(extractor.ngram_candidates())  # ngrams only
    calculate_metrics(mwe_gold, ngram_candidates, True)
    extractor.ov_filter = False """

    uni_candidates = set(extractor.unigram_candidates())
    n_candidates = set(extractor.ngram_candidates())

    true_positives, false_positives, false_negatives = compare_sets(
        gold, uni_candidates | n_candidates
    )
    save_set_to_csv(true_positives, "true_positives_cands.csv")
    save_set_to_csv(false_positives, "false_positives_cands.csv")
    save_set_to_csv(false_negatives, "false_negatives_cands.csv")
...
def main():
    ...
    gold_set = load_gold_set(str(GOLD_SET_PATH), GRAM_TYPE)
    ...
    vanilla_candidates, _ = extractor.embed_setup(
        domain=DOMAIN,
        gram_type=f"{GRAM_TYPE}_vanilla",
        use_cache=USE_CACHE,
    ...
    # for a sample of known true terms and known false positives in your MWE set
    sample_false = [
        ...
        for filter_combo in itertools.combinations(filter_names, combo_size):
            ...
            for thresholds in threshold_grid:
                ...
                for fname, threshold in zip(filter_combo, thresholds):
                    ...
                    for term in sample_true + sample_false:
                        print(f"{term}:")
                        print(
                            f"^^^^^^^^^^^ {fname}, {threshold}  = {all_filters[fname][0].get(term, "MISSING")}"
                        )
